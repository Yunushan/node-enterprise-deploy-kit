...
alias.
Normal push/PR CI runs `Test-Repository.ps1 -SkipReleaseEvidenceSelfTests` so
Windows and Linux checks stay responsive. The dedicated
`release-evidence-checks` job still runs the heavy bundle, coverage, import,
release-workflow, release-readiness, and redacted readiness-summary self-tests
once per CI run.
...
array containing `self-hosted` plus the expected target label. Do not include
any other support-matrix target ID in the same label set; one workflow run
should prove one exact target row. GitHub-hosted labels such as
`ubuntu-latest`, `ubuntu-24.04`, `windows-latest`, `windows-2022`,
`windows-2025`, `macos-latest`, and `macos-15` are rejected for real evidence
collection.
...
service managers, including service-only fallback rows such as
`target-mode-pm2-none-fallback`. Use a
...
For the least ambiguous real-host evidence handoff, generate a collection pack.
...
the staging audit run `Test-HostEvidence.ps1` against each staged row, including
uptime, Next.js runtime, reverse-proxy, collector SHA256, and workflow
provenance checks, before the release gate. The audit auto-resolves the
validator from the repository root or the default `evidence\collection-pack`
location; pass `-HostEvidenceValidatorPath` if the pack was copied elsewhere.
It still requires downloaded workflow artifacts and local-command-only evidence
from real hosts before readiness can pass.

To generate only the real-host evidence checklist for a release:
...
bundle, verifies the bundle, writes detailed release readiness JSON, and writes
...
refuses `-AllowWarnings`; final support evidence must be warning-clean. Add
...
that switch requires `-StrictCiRelease`, `-IncludeServiceOnly`, and
`-IncludeFallback`.
...
It also records top-level `supportScope` metadata, including scope kind, proof
level, selected-vs-matrix target counts, workflow-capable evidence counts, and
local-command-only evidence counts; the bundle verifier recalculates those
values from the manifest rows and current support matrix.
...
keeps its collection run identity after bundling. Each manifest row also
preserves the collection workflow dispatch evidence name, expected target,
Next.js mode, service manager, reverse proxy, minimum uptime, support matrix
path, and support matrix SHA256 fields, plus a match flag for the archived
support row.
...
`host-evidence` workflow dispatch for workflow-capable evidence rows, requires
the bundle manifest's recorded workflow dispatch dimensions to match the exact
support row and support matrix, and rejects `-AllowWarnings`, so final signoff cannot be
warning-tolerant. Targets
