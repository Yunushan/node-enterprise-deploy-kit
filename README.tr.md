...
self-testleri ile redacted readiness-summary self-testleri ayrı
`release-evidence-checks` job'ında bir kez çalışır.
...
coverage, non-synthetic/non-mock/non-sample evidence zorunlulugu, unique
evidence payload zorunlulugu, workflow applicability metadata, runtime support
metadata, provenance zorunluluklari, collector SHA256, runtime version
zorunlulugu ve minimum uptime.
...
bu switch `-StrictCiRelease`, `-IncludeServiceOnly` ve `-IncludeFallback`
gerektirir.
...
veya geniş erişimli repository'lerde bu input `false` kalmalıdır. `matrix_path`
release icin kullanılan committed support matrix'e ayarlanmalıdır; workflow
input validator'ları bunun tracked repository `.json` file olmasını ister, bu
nedenle self-hosted workspace icinde kalan untracked veya private matrix files
reddedilir. Bundle workflow'u, ayrı verifier workflow'u ve redacted summary
verifier coverage, bundle, readiness ve summary verification icin aynı relative
matrix path'i kullanır. Saved bundle verifier, manifest `matrixPath` değerinin
tracked repository `.json` file olmasını ve `matrixSha256` değerinin bu file ile
eşleşmesini de doğrular. Workflow ile toplanan her evidence row'u ayrıca
collection workflow dispatch icindeki support matrix path ve support matrix
SHA256 değerini manifest'e taşır; final gate bu değerlerin bundle matrix'i ile
eşleşmesini bekler. Coverage auditor da workflow-capable satırlarda
`matrix_path`, `-ExpectedMatrixPath` ve `-ExpectedMatrixSha256` üretir; farklı
support matrix path veya SHA256 ile toplanmış workflow evidence satırlarını
coverage olarak saymaz. Self-hosted runner'da canonical `evidence/` zaten hazirsa
`artifact_path` bos
...
dosyasını `scripts/dev/Resolve-ReleaseEvidenceBundle.ps1` ile bulur, aynı
`matrix_path` ile strict readiness kontrolünü çalıştırır, private evidence
bundle'ı yeniden yüklemez,
...
`generatedAtUtc`, `releaseClaim.requirements`,
...
`coverage.runtimeSupportTiers`, `supportMatrix.sha256`,
`supportMatrix.targetCount`, `supportMatrix.requiredMinimumUptimeHours`,
`supportMatrix.runtimeSupportTiers`,
`sourceControl.isGitRepository`, `sourceControl.commitSha`,
`bundleCi.provider`, `bundleCi.workflowName`, `bundleCi.eventName`,
`bundleCi.runId`, `bundleCi.runAttempt` ve `bundleCi.sha` gibi guvenli
claim/provenance ve
aggregate count alanlarını içerir. Final summary, gercek Git repository,
lowercase 40-character git SHA, eslesen GitHub Actions bundle SHA ve GitHub
Actions `workflow_dispatch` bundle-producing CI run kanıtını doğrular. Raw coverage satırlarını, collection komutlarını,
...
doğrular.
`scripts/dev/Invoke-SupportEvidenceReleaseWorkflow.ps1`, summary dosyasını
coverage, bundle ve readiness için kullanılan aynı `-MatrixPath` ile hemen
doğrular; böylece tek komutluk release handoff doğrulanmamış summary
yayınlayamaz.
`scripts/dev/Test-ReleaseReadinessSummary.ps1`, indirilen
`release-readiness-summary.json` dosyasını private bundle'ı okumadan doğrular;
`-RequireFinalFullMatrixReleaseClaim` ile final olmayan, tam coverage
kanıtlamayan, clean source/CI provenance veya guvenli runtime support metadata
eksik olan, strict CI full-matrix claim kind taşımayan, support matrix SHA256,
strict CI aktif proof level, target count, gerekli uptime, runtime support
tier'ları, scope flag'leri, matrix'ten türetilen evidence count'u,
workflow/local evidence count'ları, runtime aggregate count'ları, kayıtlı
bundleSupportScope target count'ları,
...
flag'leri (`supportClaimValidated`, `requireBothNextJsModes`,
`requireDeclaredServiceManagers`, `requireDeclaredReverseProxies`), coverage
warning-clean/uptime metadata'sı
(`coverage.failOnWarningsDuringCollection`,
`coverage.requiredMinimumUptimeHours`) ve
`bundleSupportScope.requiredMinimumUptimeHours` ile collection provenance
aggregate count'ları (`collectionProvenance`, `collectionCiEvidenceCount`,
...
`collectionWorkflowDispatchMatrixMismatchCount`) eksik veya eşleşmeyen ya da raw
evidence detayı içeren summary dosyalarını reddeder.
Host evidence ve coverage validation eksik, geçersiz, stale veya future-dated
`generatedAtUtc` değerlerini reddeder; bu nedenle clock drift veya elle
değiştirilmiş status dosyaları freshness window'u uzatamaz.
Varsayılan olarak
`config/support-matrix.example.json` kullanılır; release'e özel matrix
doğrulamak için `-MatrixPath` verin.
