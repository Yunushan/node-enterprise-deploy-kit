...
- Hosted Linux target-family real Next.js container coverage for Ubuntu, Debian, Linux Mint, RHEL/UBI, Oracle Linux, CentOS/CentOS Stream, Rocky Linux, AlmaLinux, Fedora, and Alpine: checksum-verified Node.js on glibc, signed Alpine `apk` Node.js, pinned Next.js/React fixture versions from `config/nextjs-integration-versions.json`, both package modes, extraction, and live HTTP output
...
- Hosted real Next.js integration coverage on Ubuntu, Windows Server 2022, Windows Server 2025, and macOS 15: builds the pinned Next.js/React fixture versions from `config/nextjs-integration-versions.json`, packages both modes, strictly imports each artifact, verifies provenance and live HTTP output, and runs a temporary launchd service lifecycle on macOS
